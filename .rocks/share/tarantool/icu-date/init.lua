...
local cnv = call_fn('ucnv_open', 'UTF-8', status_ptr)
if status_ptr[0] ~= icu.U_ZERO_ERROR then
  error("Failed to call ucnv_open('UTF-8')")
end
...
local function uchar_to_string(uchar, length)
  local str = ffi.new('char[?]', length + 1)
  local status, needed_length = call_fn_status("ucnv_fromUChars", cnv, str, length, uchar, -1, status_ptr)
  if status == icu.U_BUFFER_OVERFLOW_ERROR then
    length = needed_length + 1
    str = ffi.new('char[?]', length)
    call_fn_status("ucnv_fromUChars", cnv, str, length, uchar, -1, status_ptr)
  end
  local rc, err = check_status(status_ptr[0], true)
  if rc == nil then
      return nil, err
  end
...
function _M:get_time_zone_id()
  ...
  return uchar_to_string(result, result_length)
end
...
local function serialize_cal(self)
  return _M.format(self, _M.formats.iso8601())
end

function _M:__tostring()
  check_self_type(self, "__tostring")
  return serialize_cal(self)
end

function _M:__serialize()
  check_self_type(self, "__serialize")
  return serialize_cal(self)
end
